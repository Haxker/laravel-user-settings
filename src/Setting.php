...
class Setting {
	...
	public function __construct() {
		...
		$this->cache = config('laravel-user-settings.cache');
	...
	public function save($constraint_value = null) {
		...
		if ($this->dirty[$constraint_value]) {
			...
			if ($this->cache) {
				\Cache::forget('settings_for_' . $constraint_value);
			}
	...
	public function load($constraint_value = null) {
		...
		if ($this->cache) {
			$json = \Cache::rememberForever('settings_for_' . $constraint_value, function () use ($constraint_query) {
